...
<main role="main" class="container">
  <div class="d-flex align-items-center p-3 my-3 text-white-50 bg-purple rounded shadow-sm">
    ...
    <div class="lh-100">
	  ...
	  <button type="button" class="btn btn-primary" onclick="sendVip(1)">发放1天会员</button>
	  <button type="button" class="btn btn-secondary" onclick="sendVip(7)">发放7天会员</button>
	  <button type="button" class="btn btn-info" onclick="sendVip(30)">发放30天会员</button>
	  <button type="button" class="btn btn-danger" onclick="sendVip(365)">发放1年会员</button>
...
<script type="text/javascript">
	function sendVip(day){
		var isok = confirm("确定要发放"+ day +"天会员吗？");
		if(isok){
			$.ajax({
			   type: "POST",
			   url: "${indexpath}/manage/sendVip.action",
			   data: {"day":day,"userID":${user.id}},
			   dataType:"json",
